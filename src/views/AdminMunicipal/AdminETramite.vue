...
export default {
  ...
  mounted() {
    try {
      ...
      if (Cookies.get('administrador') == false) {
        axios
          .get(
            'http://localhost:7065/Tramites/readOficinas?id_oficina=' +
        ...
        axios
          .get('http://localhost:7065/Tramites/readAdministrador')
          .then(
  ...
  methods: {
    ...
    doSomething(id) {
      Cookies.set('id_tramite', id), this.$router.push('/VerificaTramite')
    },
  },
}
</script>
